<template>
  <div :class="show ? 'active' : 'sidebar-wrapper'">
    <div>
      <div style="border-bottom: 1px solid grey; width:220px;">
        <router-link v-if="authUser.image" to="/dashboard/profile">
          <div
            class="sidebar-img mt-4"
            :style="{ backgroundImage: 'url(' + authUser.image + ')'}"
          >&nbsp;</div>
        </router-link>
        <router-link v-else to="/dashboard/profile">
          <svg xmlns="http://www.w3.org/2000/svg" width="95" class viewBox="0 0 53 53">
            <path
        ...
        <div style="text-align: center!important; text-size: 12px !important;">
          <h4>{{authUser.firstname}}</h4>
          @{{authUser.firstname}}
        </div>
        <div
          class="d-flex flex-row justify-content-center mb-3 align-items-center"
          style="margin: auto!important; width:200px;"
        >
          <div class="text-sec">
            <span class="num">
